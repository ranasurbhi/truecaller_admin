...
class _CampaignManagementWebScreenState
  ...
  void _exportCampaigns() {
    ...
    for (final c in campaigns) {
      final teamNames =
          (c["assignedTeam"] as List).map((t) => t["name"]).join(" | ");
        ...
        children: [
          ...
          Expanded(
              ...
              child: Text(c["duration"],
                  style:
                      const TextStyle(fontSize: 12, color: Colors.grey))),
          Expanded(flex: 3, child: _assignedTeam(c["assignedTeam"])),
        ],
      ),
    );
  }

  Widget _assignedTeam(List team) {
    if (team.isEmpty) return const Text("-", style: TextStyle(color: Colors.grey));

    return Row(
      children: team.take(3).map<Widget>((t) {
        return Padding(
